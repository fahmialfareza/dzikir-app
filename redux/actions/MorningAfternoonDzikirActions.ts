...
import { setEveningAfternoonDzikir, setMorningAfternoonDzikir } from '../reducers/morningAfternoonDzikir';
  ...
  static getMorningAfternoonDzikir() {
    return async (dispatch: any) => {
      try {
        ...
        dispatch(setMorningAfternoonDzikir(data.filter((dzikir) => dzikir.time === 'pagi' || dzikir.time === '')));
        dispatch(setEveningAfternoonDzikir(data.filter((dzikir) => dzikir.time === 'petang' || dzikir.time === '')));
      } catch (error) {
        // Add toastify
        throw error;
      }
